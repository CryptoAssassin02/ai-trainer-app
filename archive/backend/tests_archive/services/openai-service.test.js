...
// Ensure the actual service implementation is loaded
jest.unmock('../../services/openai-service');

// --- Now Require modules AFTER mocks ---
// Import the Class definition now
...
describe('OpenAIService', () => {
  ...
  beforeEach(() => {
    ...
    // Instantiate the service here, AFTER mocks are set up
    serviceInstance = new OpenAIService();

    // REMOVE THIS - no longer using the mock service
    // Set up basic responses for commonly used methods
    // OpenAIService.mockGenerateChatCompletion.mockResolvedValue('Hello User!');
    // OpenAIService.mockGenerateEmbedding.mockResolvedValue(mockEmbedding);
  });
  ...
  describe('Initialization', () => {
    ...
    test('should be defined', () => {
      // Now test the instance created in beforeEach
  ...
  describe('generateChatCompletion', () => {
    ...
    test('should not retry for non-retryable API errors (e.g., 400)', async () => {
      ...
      await expect(serviceInstance.generateChatCompletion(messages)).rejects.toThrow(MockAPIErrorForTests);
      await expect(serviceInstance.generateChatCompletion(messages)).rejects.toHaveProperty('status', 400);
    ...
    test('should throw original error for unexpected non-API errors', async () => {
      ...
      await expect(serviceInstance.generateChatCompletion(messages)).rejects.toThrow('Something went wrong');
    ...
    test('should throw error for invalid response structure', async () => {
      ...
      await expect(serviceInstance.generateChatCompletion(messages)).rejects.toThrow('Invalid response structure from OpenAI Chat Completion');
    ...
    test('should handle API timeouts after retries', async () => {
      // Simulate a timeout error (using a generic Error for simplicity, 
      // or a specific timeout error if the SDK provides one)
      const timeoutError = new Error('Request timed out');
      // Ensure the status is NOT typically retryable unless timeouts are handled specifically
      // If timeout IS retryable per config, this mock needs to reject multiple times.
      _mockChatCompletionsCreate.mockRejectedValue(timeoutError);
      ...
      // Expect the original timeout error to be thrown after retries fail
      expect(caughtError).toBe(timeoutError);
      // Service retries based on status code, generic errors aren't retried by default
      expect(_mockChatCompletionsCreate).toHaveBeenCalledTimes(1);
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('An unexpected non-APIError occurred during OpenAI Chat Completion.'),
        timeoutError
       ...
       expect(logger.error).toHaveBeenCalledWith(
          'Error during OpenAI Chat Completion (Attempt 1)',
          { error: timeoutError.message, status: undefined } // Status might be undefined
      );
       // Check the final failure log
       expect(logger.error).toHaveBeenCalledWith(
           'Error during OpenAI Chat Completion (Attempt 1)',
           { error: timeoutError.message, status: undefined }
       );
    });
  });
  ...
  describe('generateEmbedding', () => {
    test('should use default embedding model from test config', async () => {
      ...
      // Use the instance
      await serviceInstance.generateEmbedding(inputText);
    ...
    test('should override default embedding model with provided options', async () => {
      ...
      // Use the instance
      await serviceInstance.generateEmbedding(inputText, options);
      expect(_mockEmbeddingsCreate).toHaveBeenCalledWith(expect.objectContaining({
        model: options.model,
        input: inputText,
      }));
    ...
    test('should return embedding vector for single string input', async () => {
      _mockEmbeddingsCreate.mockResolvedValue(mockSuccessResponseEmbedSingle);
      // Use the instance
      const result = await serviceInstance.generateEmbedding(inputText);
     ...
     test('should return array of embedding vectors for array input', async () => {
      ...
      // Use the instance
      const result = await serviceInstance.generateEmbedding(inputTexts);
    ...
    test('should not retry for non-retryable API errors (e.g., 401)', async () => {
      ...
      await expect(serviceInstance.generateEmbedding(inputText)).rejects.toThrow(MockAPIErrorForTests);
      await expect(serviceInstance.generateEmbedding(inputText)).rejects.toHaveProperty('status', 401);
     ...
     test('should throw error for invalid response structure (missing data)', async () => {
      ...
      await expect(serviceInstance.generateEmbedding(inputText)).rejects.toThrow('Invalid response structure from OpenAI Embeddings');
    ...
    test('should throw error for invalid response structure (missing embedding)', async () => {
      ...
      await expect(serviceInstance.generateEmbedding(inputText)).rejects.toThrow('Missing embedding in response data at index 0');
    ...
    test('should handle API timeouts for embeddings after retries', async () => {
      const timeoutError = new Error('Embedding request timed out');
      _mockEmbeddingsCreate.mockRejectedValue(timeoutError);
      ...
      expect(_mockEmbeddingsCreate).toHaveBeenCalledTimes(1);
       expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('An unexpected non-APIError occurred during OpenAI Embedding Generation.'),
        timeoutError
      );
      // Check the initial attempt error log
       expect(logger.error).toHaveBeenCalledWith(
